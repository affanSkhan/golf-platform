...
create or replace function public.trim_golf_scores() returns trigger language plpgsql security definer set search_path=public as $$
begin
  delete from public.golf_scores
  where user_id=new.user_id
    and id not in (select id from public.golf_scores where user_id=new.user_id order by score_date desc,created_at desc limit 5);
  return new;
end;
$$;
create trigger golf_scores_keep_latest_five after insert on public.golf_scores for each row execute function public.trim_golf_scores();
...
create policy "subscriptions admin insert" on public.subscriptions for insert with check (public.is_admin());
